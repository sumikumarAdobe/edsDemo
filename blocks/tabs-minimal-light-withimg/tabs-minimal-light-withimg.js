...
export default async function decorate(block) {
  ...
  tabHeadings.forEach((tab, i) => {
    ...
    // group panel content into image + text wrappers for side-by-side layout
    // (the content cell is the second cell; the first cell is the tab title)
    const content = tabpanel.children[1];
    if (content) {
      const imgPara = content.querySelector('picture')?.closest('p');
      const imageWrap = document.createElement('div');
      imageWrap.className = 'tabs-minimal-light-withimg-image';
      const textWrap = document.createElement('div');
      textWrap.className = 'tabs-minimal-light-withimg-text';
      [...content.children].forEach((node) => {
        if (node === imgPara) imageWrap.append(node);
        else textWrap.append(node);
      });
      content.textContent = '';
      if (imageWrap.children.length) content.append(imageWrap);
